...
## Now（当前焦点 · 2026-06-09）
「家庭 + 用户」多租户改造三轮 + 个人中心已全部落地并真机验证（ADR-008/009）：隔离键 openid → family，每云函数 assertMember 守卫；个人中心（个人档案昵称头像 / 家庭切换·管理·邀请码 / 用户·隐私协议）。全代码库经对抗式评审，修掉 user `_openid` 隔离键漏写等一批真问题。随后做底部导航重构（4 tab + 中央凸起「＋」全局录入键，提醒 / 药品合并「健康」，录入迁独立页，自定义 tabBar，ADR-010）、家庭成员昵称 / 头像读时关联 users 修复、构建瘦身（产物不再拷 node_modules，部署改云端安装依赖，根治 DevTools 刷新风暴）。当前 8 云函数 + 9 集合，版本 0.3.0。

# ARCHITECTURE · PetsLog
<!-- 内部结构 + 不能动什么。决策理由→DECISIONS(这里只放结论+链接)；对外契约→SPEC。 -->

## 组件 + 数据流
```
微信小程序 (uni-app 一套代码，目标端先小程序，后可扩 APP/H5)
  底部 4 tab（自定义 tabBar）+ 中央凸起「＋」全局录入键（任意 tab 可录，ADR-010）
  ├─ 宠物（首页 tab）：档案卡左右滑轮播（一屏一宠：头像/名/性别/物种品种/年龄/最新体重/陪伴天数/体重趋势/简介，末张「＋ 添加宠物」，点卡进详情，ADR-022/025）+ 到期提醒横幅（去常驻输入条，录入走中央＋）
  ├─ 时间线：单宠 / 全宠健康流水（症状/用药/疫苗/驱虫/体重/就医/养护 8 桶，含医院/费用/病程，可按病程筛，分页加载），可截图给兽医；物种口径 8 枚举（猫/狗/兔/小宠/鸟/爬宠/鱼/其他，ADR-023，猫狗保留完整医疗语义、其余走统一记录模型）
  ├─ 健康：顶部分段 [提醒 | 药品 | 主粮]，提醒（到期高亮）+ 家庭药品库存 + 主粮台账（家庭级喂粮史，增删改查 + 当前在喂高亮，ADR-014）
  ├─ 我的 / 个人中心：个人档案昵称头像 / 家庭管理·切换·邀请码·成员名头像（读时关联 users）/ 用户·隐私协议
  ├─ ＋ 录入页（pages/record）：自然语言 → AI 解析 → 二次确认 → 落库（独立全屏页）
  └─ 宠物档案（卡片点入）：编辑 / 删除 / 体重曲线 / 一键截图给兽医 / 🪪 档案卡分享图（ADR-021/025）
        │ wx.cloud.callFunction（自动带 openid + 客户端注入 active family_id，免自建登录态）
        ▼
微信云开发 (Serverless)
  ├─ 云函数（11）：parseRecord 解析 / saveRecord 落库 / pets·timeline·meds·reminders·foods CRUD / family 家庭+鉴权守卫 / user 个人档案 / attachment 附件配额+级联 / importNotion 一次性历史导入 / 频率限制
  ├─ 云数据库（文档型，11 集合）：pets / records / meds / reminders / foods / parse_log / att_log / families / family_members / invites / users，按 family 隔离（users 按 openid）
  └─ 云存储：记录附件（图片 / 视频 / PDF，家庭 ≤1GB，ADR-011）+ 宠物 / 用户头像
        │ 云函数 outbound（Node，不受小程序合法域名白名单约束）
        │ POST https://ark.cn-beijing.volces.com/api/coding/v3/chat/completions
        │ （OpenAI 兼容，唯一机密 ARK_API_KEY 存 config.local.js / 云函数环境变量）
        ▼
火山方舟 Coding Plan（doubao-seed-2.0-pro，直连，ADR-016）
```
> API Key 属机密不入库（见 DEPLOYMENT）；端点 / 模型为公开默认值进代码，ARK_BASE_URL / ARK_MODEL 可覆盖。

录入主链路：用户说「示例猫今天吐了，体重 4.2kg」→ 云函数组 prompt（含已有宠物名单）调方舟 → LLM 返回强制 JSON（宠物身份/时间/事件类型/用药/体重）→ 云函数落库到对应宠物时间线 → 小程序刷新。

> 此处只描述当前架构结论；自录入重做 / 多物种 / 多宠批量 / 家庭同步加固以来的逐项演进，见 DECISIONS ADR-014~036。

## 关键技术选型
- **前端 uni-app**：一套代码多端，AI 最擅长生成。目标端先微信小程序。
- **后端微信云开发（非自建服务器）**：无服务器 / 无备案域名 / 自带登录态。→ DECISIONS ADR-002。
- **LLM 直连火山方舟 Coding Plan（OpenAI 兼容，doubao-seed-2.0-pro）**：消灭自建网关中转单点；换模型走 ARK_MODEL 配置。→ DECISIONS ADR-016（反转 ADR-003）。
- **录入 = 小程序内自然语言输入框（非微信机器人 / 非填表）**：→ DECISIONS ADR-001。
- **UI 设计令牌**：全局 CSS 自定义属性挂 `App.vue` 的 `page`，各页 `var(--c-*)` 继承，一处改主题全站联动（温暖治愈方向）。→ DECISIONS ADR-007。
- **底部导航 = 4 tab + 中央凸起「＋」全局录入键**：自定义 tabBar 用微信原生组件四件套 `src/custom-tab-bar/index.{js,json,wxml,wxss}`（uni-app 当拷贝目标，不编译 .vue；选中态各 tab 独立实例按路由算）。录入从首页常驻输入条迁到独立录入页 `pages/record`，提醒 / 药品合并「健康」分段。→ DECISIONS ADR-010。

## 禁改项 / Forbidden Refactors
- **LLM key 不得移到前端**：必须留在云函数环境变量（红线，见 CORE）。
- **数据隔离不得绕过**：已从 openid 隔离迁到 family 隔离（ADR-008，34-agent 审计四不变式全绿 + isolation.e2e 67 断言，ADR-033/034 家庭数据同步加固后）。每个云函数必须入口先 `assertMember(openid, family_id)` 再读写，客户端传入的 family_id 一律不可信；不得跨家庭读写。assertMember 多副本同步红线见 CONVENTIONS。
- **AI 严格定位为「结构化提取机器」**：system prompt + 强制 JSON 不得放开成自由对话（防滥用 + 控成本 + 合规）。

...
微信小程序 (uni-app 一套代码，目标端先小程序，后可扩 APP/H5)
  ...
  ├─ 时间线：单宠 / 全宠健康流水（症状/用药/疫苗/驱虫/体重/就医/养护 8 桶，含医院/费用/病程，可按病程筛，分页加载），可截图给兽医
...
- **数据隔离不得绕过**：已从 openid 隔离迁到 family 隔离（ADR-008，13-agent 审计四不变式全绿 + isolation.e2e 64 断言）。每个云函数必须入口先 `assertMember(openid, family_id)` 再读写，客户端传入的 family_id 一律不可信；不得跨家庭读写。assertMember 多副本同步红线见 CONVENTIONS。

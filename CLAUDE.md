...
## 当前状态（2026-06-28 · 体验版 0.4.13 已发布 · ADR-036 语音改键盘自带 + AIREADME 全面回填对齐）
...
- 药品按类型区分 + 类型筛选（ADR-035，**已部署 meds/saveRecord/parseRecord + 回填海乐妙→驱虫 · 待真机验**，体验版 0.4.13）：内测反馈「添加的药要区分下类型、做个过滤」。原话「仨 tab 都有」live DB 验尺证伪（海乐妙只在 meds 一处、测试家庭 reminders/foods 皆空、三 tab 查三独立集合无重复）→ 真问题＝药品 tab 缺类型结构。meds 加 `type`（枚举 用药/疫苗/驱虫/养护/其它，复用 reminders TYPES，缺/非法默认「用药」）；落库三入口同源 normMedType（parseRecord prompt 出 med_type + normalize 白名单透传、saveRecord med_stock、meds add/update）；前端药品 tab 加筛选 chip 行（动态显在库类型、≥2 种才显）+ 每卡类型 chip + 录入/编辑各加类型 picker；`backfill_type` 维护 action（admin 守卫+幂等），存量海乐妙走 wxdump 服务端直更。13 套绿（新增 parseRecord.normalize 守白名单透传 + 变异验证）；pre-commit 2-agent（前端 0 confirmed、后端 2 should-fix + 1 nit 全修：补 normalize 透传测堵假绿 + backfill 加 admin 守卫/limit）。
- 语音录入改键盘自带听写（ADR-036，**已 commit 4cfe129 + push origin/main 9faf3a1 · 零后端 · 待真机验**）：欠两轮的三入口卡「语音」占位转正。微信服务市场「一句话识别」**三通道实测全死**（服务端 `wxa/servicemarket` 10+ 发 100% `9301002` 上游故障透传 / 客户端 `invokeService` `ok`+`data:""` 真机复现 / 异步 `9301013` 不支持；Service ID `wxa8386175898e12c9`+`SentenceASR` 经服务详情页核对正确＝坏的是微信代理壳非我方参数；探针 `tools/wxdump/asr_*.mjs` 服务端自验）→ 弃之，`record.vue`「语音」入口转正＝聚焦 NL 输入框弹系统键盘、键盘 🎤 听写、文字走现有 parseRecord、零后端零成本；腾讯云直连留未来。详见 `AIREADME/DECISIONS` ADR-036 + MEMORY「语音录入调研」。
- AIREADME 全面回填对齐 + 破折号清零（**docs 9faf3a1**）：回顾发现摘要类文档（INDEX/CORE/CONVENTIONS/ARCHITECTURE/SPEC/DEPLOYMENT/PRD/ROADMAP）普遍漂移（INDEX 最甚、冻在 0.3.0/ADR-013/8 函数 9 集合），台账类（DECISIONS/MEMORY/CHANGELOG）靠 append-only 反而最准 → 11-agent workflow 并行起草、一次性 re-sync 全 12 文件到 0.4.13/ADR-036/11 函数 11 集合/方舟直连/8 物种 + 全仓破折号清零（连带清掉旧 ADR ~17 处历史破折号）。

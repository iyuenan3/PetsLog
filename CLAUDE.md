...
- 附件轮2（v0.3.2）：记录挂图片 / 视频 / PDF（attachment 云函数服务端复核真实体积；配额：单条 ≤9、家庭 ≤1GB、日 ≤200MB）；录入卡片 + 新记录详情页双入口；缩略图省 CDN；删记录 / 解散家庭 / 换头像级联清理云存储，见 ADR-011。轮3 Notion 历史导入为下一轮。
- 三阶段演进：v1 Cursor（已下线）→ v2 OpenClaw（未上生产）→ v3 本仓库。详见 `AIREADME/CORE`。
- 下一步：轮3 Notion 历史导入（结构化直灌）；邀友内测 + 解析准确率调优 + 提醒真推送（订阅消息）+ token 轮换。见 `AIREADME/ROADMAP`。

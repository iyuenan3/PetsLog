...
- 下一步：**真机内测回归**（录入三路 / 时间到分 / 多物种 + 养护录入 / 档案卡富化版式 ♂♀ 字形 / 体重 band / 过渡动画 / 病程视图；图标系统 ADR-026 已真机验过）→ 加体验成员 + 发码邀友；**主粮多宠化（ADR-027，已实现 c9d1117，待部署 foods+pets 云函数 + 待跑 backfill_foods 迁移 12 条：foods 物种默认 + 单宠覆盖 + brand/model 拆分 + 台账按物种分组 + pet.vue 当前主粮）**；后续 ADR-020 Phase 2 解析评测 + pet.vue 该宠病程入口 + 提醒真推送（订阅消息）+ 语音录入；UI 按需定点打磨。见 `AIREADME/ROADMAP`。
